...
with mlflow.start_run(run_name='RF Optuna') as run:
    ...
    # log tags
    mlflow.set_tags(tags={'Project': 'In class 5 RF Optuna',
                         'Opimizer': 'Optuma',
...
RF_params_new = study_new.best_params
RF_rmse_new = study_new.best_value

with mlflow.start_run(run_name='RF Optuna FS') as run:
    #log best params
    mlflow.log_params(RF_params_new)
    
    #define the model
    model_new = RandomForestRegressor(**RF_params_new)
    ...
    # log score
    mlflow.log_metric('rmse', RF_rmse_new)
    
    # log tags
    mlflow.set_tags(tags={'Project': 'In class 5 RF Optuna FS',
                         'Opimizer': 'Optuma',
                         'Model_family': 'Random forest',
                         'feature_version': 2})
    mlflow.end_run()