...
ET_rmse = study.best_value

mlflow.set_experiment('Inclass5')
...
with mlflow.start_run(run_name='ET Optuna') as run:
    ...
    # log score
    mlflow.log_metric('rmse', ET_rmse)
    
    # log tags
    mlflow.set_tags(tags={'Project': 'In class 5 ET Optuna',
                         'Opimizer': 'Optuma',
...
model = ExtraTreesRegressor(**ET_params).fit(X, y)
...
ET_params_new = study_new.best_params
ET_rmse_new = study_new.best_value

with mlflow.start_run(run_name='ET Optuna FS') as run:
    #log best params
    mlflow.log_params(ET_params_new)
    
    #define the model
    model_new = ExtraTreesRegressor(**ET_params_new)
    ...
    # log score
    mlflow.log_metric('rmse', ET_rmse_new)
    
    # log tags
    mlflow.set_tags(tags={'Project': 'In class 5 ET Optuna FS',
                         'Opimizer': 'Optuma',
                         'Model_family': 'Extra Trees',
                         'feature_version': 2})
    mlflow.end_run()