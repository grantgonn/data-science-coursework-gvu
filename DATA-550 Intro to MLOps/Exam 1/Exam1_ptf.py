...
with mlflow.start_run(run_name='LGB Optuna') as run:
    ...
    # log tags
    mlflow.set_tags(tags={'Project': 'Exam 1 LGB Optuna',
                         'Opimizer': 'Optuma',
                         'Model_family': 'LGB',
                         'feature_version': 1})
    mlflow.end_run()
...
LGB_params_new = study_new.best_params
LGB_rmse_new = study_new.best_value

if(LGB_rmse_new < LGB_rmse):
    with mlflow.start_run(run_name='LGB Optuna FS') as run:
        #log best params
        mlflow.log_params(LGB_params_new)
        
        #define the model
        model_new = LGBMRegressor(**LGB_params_new)
        ...
        # log score
        mlflow.log_metric('rmse', LGB_rmse_new)
        
        # log tags
        mlflow.set_tags(tags={'Project': 'Exam 1 LGB Optuna FS',
                            'Opimizer': 'Optuma',
                            'Model_family': 'LGB',
                            'feature_version': 2})
        mlflow.end_run()