...
with mlflow.start_run(run_name='XGB Optuna') as run:
    ...
    # log tags
    mlflow.set_tags(tags={'Project': 'Exam 1 XGB Optuna',
                         'Opimizer': 'Optuma',
                         'Model_family': 'XGB',
                         'feature_version': 1})
    mlflow.end_run()
...
XGB_params_new = study_new.best_params
XGB_rmse_new = study_new.best_value

if(XGB_rmse_new < XGB_rmse):
    with mlflow.start_run(run_name='XGB Optuna FS') as run:
        #log best params
        mlflow.log_params(XGB_params_new)
        
        #define the model
        model_new = XGBRegressor(**XGB_params_new)
        ...
        # log score
        mlflow.log_metric('rmse', XGB_rmse_new)
        
        # log tags
        mlflow.set_tags(tags={'Project': 'Exam 1 XGB Optuna FS',
                            'Opimizer': 'Optuma',
                            'Model_family': 'XGB',
                            'feature_version': 2})
        mlflow.end_run()