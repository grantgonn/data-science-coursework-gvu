...
def objective(trial):
    ...
    total_weight = rf_weight + et_weight + gb_weight + xgb_weight
    ...
    vote_md = VotingRegressor(estimators=base_models, weights=[rf_weight, et_weight, gb_weight, xgb_weight], n_jobs=-1, verbose=0)
...
w_total = best_weights['rf_weight'] + best_weights['et_weight'] + best_weights['gb_weight'] + best_weights['xgb_weight']
...
# define the voting regressor
vote_md = VotingRegressor(estimators=base_models, weights=[best_weights['rf_weight'], best_weights['et_weight'], best_weights['gb_weight'], best_weights['xgb_weight']], n_jobs=-1, verbose=False)
...
with mlflow.start_run(run_name='Voting Regressor') as run:
    ...
    # log tags
    mlflow.set_tags(tags={'Project': 'In class 8 Voting Optuna',
