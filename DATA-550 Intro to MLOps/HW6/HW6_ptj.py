...
# define meta model
meta_model = MLPRegressor(hidden_layer_sizes=(64, 64))

# define the stacking regressor
stack_md = StackingRegressor(estimators=base_models, final_estimator=meta_model, n_jobs=-1, cv=KFold(n_splits=5, shuffle=True, random_state=10))

# compute the cross val score
scores = -1* cross_val_score(stack_md, X, y, cv=skf, scoring='neg_mean_absolute_error', n_jobs=-1).mean()

# setting the experiment
mlflow.set_experiment('HW6')

with mlflow.start_run(run_name='Stacking Regressor MLP') as run:
    #define the model
    model = stack_md
    ...
    # log tags
    mlflow.set_tags(tags={'Project': 'HW6 Stacking MLP',
