...
def plot_perm_imp_depen(X, y, model):
    ...
    fig.suptitle('LGB Partial Dependence')
    ...
    plt.savefig('LGB_partial_dependence.png')
    plt.show()
    
# running permutation importance and partial dependence
plot_perm_imp_depen(X, y, LGBMRegressor(**LGB_params))

with mlflow.start_run(run_name='LGB Optuna') as run:
    # log feature importance and partial dependence
    mlflow.log_artifact('LGB_importance.png')
    mlflow.log_artifact('LGB_partial_dependence.png')
    ...
    # log tags
    mlflow.set_tags(tags={'Project': 'HW4 LGB Optuna',
                         'Opimizer': 'Optuma',
                         'Model_family': 'LGB',
                         'feature_version': 1})
    mlflow.end_run()
