...
def plot_perm_imp_depen(X, y, model):
    ...
    plt.savefig('ET_importance.png')
    plt.show()
    
    #plot partial dependence
    fig, ax1 = plt.subplots(3, 3,  figsize=(20, 20))
    fig.suptitle('ET Partial Dependence')
    ...
    plt.savefig('ET_partial_dependence.png')
    plt.show()
    
# running permutation importance and partial dependence
plot_perm_imp_depen(X, y, ExtraTreesRegressor(**ET_params))

with mlflow.start_run(run_name='ET Optuna') as run:
    # log feature importance and partial dependence
    mlflow.log_artifact('ET_importance.png')
    mlflow.log_artifact('ET_partial_dependence.png')
    ...
    # log tags
    mlflow.set_tags(tags={'Project': 'HW4 ET Optuna',
                         'Opimizer': 'Optuma',
